...
a) Elimina las copias de datos entre procesos
b) Es más fácil de programar
c) Incluye sincronización automática
...
a) Cuando se necesita comunicación secuencial simple
b) Cuando los procesos están en diferentes máquinas
c) Siempre, es superior en todo sentido
d) Cuando se comparten grandes volúmenes de datos y se necesita velocidad
...
a) Se expande automáticamente el archivo
b) Se crea un mmap de tamaño 0
...
b) Pasándolo como argumento a Process()
c) Escribiéndolo en un archivo temporal
d) Se hereda automáticamente a través de `fork()`
...
c) Un pipe con buffer de tamaño 0
d) Un array de enteros de tamaño 0
...
c) Falta llamar a v.close()
d) Value no soporta el tipo 'i'
...
a) `close()` es para el productor, `unlink()` para el consumidor
b) Son sinónimos
c) `close()` desconecta al proceso, `unlink()` elimina la memoria del sistema
d) `close()` elimina la memoria, `unlink()` desconecta al proceso

### Pregunta 14
¿Cómo se conecta un proceso a una SharedMemory existente?

a) `SharedMemory(create=True, name="nombre")`
b) `SharedMemory(attach="nombre")`
c) `SharedMemory(name="nombre")` (sin create=True)
d) `SharedMemory.connect("nombre")`
...
1. **a** - Elimina las copias de datos entre procesos. Los procesos acceden directamente a la misma región de memoria física.
2. **d** - 2 copias: del proceso escritor al buffer del kernel, y del buffer del kernel al proceso lector.
3. **a** - Race conditions. Sin sincronización, múltiples procesos pueden leer/escribir simultáneamente y producir resultados inconsistentes.
4. **d** - Cuando se comparten grandes volúmenes de datos y se necesita velocidad. Para datos pequeños o comunicación simple, pipes/colas son más prácticos.

### Parte 2: mmap
5. **b** - Crear un mmap anónimo de 1024 bytes. El `-1` como file descriptor indica que no hay archivo asociado.
6. **a** - `ACCESS_COPY`. Crea una copia privada donde los cambios no se propagan al archivo original.
7. **c** - Se produce un error. No se puede mapear un archivo vacío; hay que darle tamaño antes de mapearlo.
8. **d** - Se hereda automáticamente a través de `fork()`. El hijo recibe una copia del mapping del padre, apuntando a la misma memoria física.
...
13. **c** - `close()` desconecta al proceso de la memoria compartida (pero sigue existiendo), `unlink()` elimina el bloque de memoria del sistema operativo.
14. **c** - `SharedMemory(name="nombre")` sin `create=True`. Esto se conecta a un bloque existente en lugar de crear uno nuevo.
